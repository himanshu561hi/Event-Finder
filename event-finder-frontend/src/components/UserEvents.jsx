...
const UserEvents = () => {
    ...
    // Fetches events created by the currently authenticated user
     const fetchUserEvents = async () => {
        ...
        if (!isAuthenticated || !user?._id) { // <-- FIX: Check for user?._id existence
            setLoading(false);
            return;
        }
        ...
        setLoading(true);
        try {
            // CRITICAL FIX: Use user._id (Mongoose ID) instead of user.id (Google ID)
            const response = await axios.get(`http://localhost:5050/api/events?ownerId=${user._id}`);
    ...
    const handleDelete = async (eventId) => {
        ...
        try {
            // Securely call the DELETE API endpoint with credentials
            await axios.delete(`http://localhost:5050/api/events/${eventId}`, { withCredentials: true });
            
            // Instantly update the UI by removing the deleted event from the state
    ...
    return (
        <div>
            {userEvents.length > 0 ? (
                        ...
                        <div key={event._id} className="relative bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden group flex flex-col">
                            ...
                            <div className="p-4 lex flex-col">
                                ...
                                <div className="mt-auto">
                                    {/* Action Buttons */}
                                    <div className="pt-3 border-t border-gray-100 flex justify-between space-x-2">
                                        <Link 
                ...
                // Message shown when the user has not created any events
