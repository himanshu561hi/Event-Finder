...
const VerifyProfile = () => {
    ...
    const [formData, setFormData] = useState(initialFormData);
    // ❌ Hata diya: [status, setStatus] -> Hum sirf currentDBStatus use karenge
    const [message, setMessage] = useState('');
    const [submitting, setSubmitting] = useState(false);
    
    // Naya state file input ko manually reset karne ke liye
    const [fileInputKey, setFileInputKey] = useState(Date.now()); 


    // useEffect to pre-fill existing user data for Edit Mode
    useEffect(() => {
        if (!loading && user?.verificationDetails?.submittedAt) {
            const data = user.verificationDetails;
        ...
        } else if (!loading && !user?.verificationDetails?.submittedAt) {
             // Agar user naya hai aur koi data nahi hai, toh form ko reset rakhein
             setFormData(initialFormData);
        }
    }, [loading, user, isEditMode]);


    // Status check ko user ke DB data se sync karein (Declarative Status)
    const currentDBStatus = loading ? 'loading' : user.verifiedProfile ? 'verified' : (user.verificationDetails?.submittedAt ? 'submitted' : 'pending');
    ...
    const handleSubmit = async (e) => {
        ...
        // Document Check (Edit mode ya first submission, document hamesha required hai)
        if (!formData.documentFile) {
            setMessage("Please select a document. Re-upload is required for security validation.");
            return;
        }
        ...
        try {
            ...
            // Clean up: File input ko reset karna aur documentFile state ko null karna
            setFormData(prev => ({ ...prev, documentFile: null }));
            setFileInputKey(Date.now()); // File input ko reset karne ke liye key change karein
            
            // Redirect to Settings page (ya home page, jahan user status dekhe)
            navigate('/settings'); 
    ...
    const renderStatusBadge = (statusValue) => {
        switch (statusValue) {
            ...
            case 'submitted':
                return <span className="inline-block px-3 py-1 text-sm font-semibold text-white bg-blue-600 rounded-full">⏳ Under Review</span>;
            case 'rejected':
                // Note: Agar backend rejected status store karta hai toh usse yahan dikhayein
                return <span className="inline-block px-3 py-1 text-sm font-semibold text-white bg-red-600 rounded-full">❌ Rejected (Resubmit)</span>;
            case 'pending':
            default:
                return <span className="inline-block px-3 py-1 text-sm font-semibold text-gray-700 bg-yellow-300 rounded-full">❗ Action Required</span>;
    ...
    return (
        <div className="max-w-xl mx-auto mt-6 p-6 bg-white rounded-xl shadow-2xl">
            ...
            <div className="p-4 mb-6 border-l-4 border-blue-500 bg-blue-50">
                ...
                <p className="text-sm text-gray-600 mt-2">
                    {currentDBStatus === 'submitted' ? 'Your details are currently under review. Please wait for an update.' : 
                     isEditMode ? 'Update your information and re-submit your document.' : 'Please fill out the form below.'}
                </p>
            </div>
            
            {/* 2. Verification Form */}
            {/* Form tabhi dikhega jab status 'verified' na ho */}
            {currentDBStatus !== 'verified' && (
                <form onSubmit={handleSubmit} className="flex flex-col gap-4 p-4 border rounded-lg">
                    ...
                    {/* DOCUMENT UPLOAD */}
                    <p className="text-sm font-semibold text-red-600">
                        {isEditMode || currentDBStatus === 'submitted' ? 'Required:' : 'Mandatory:'} Please re-upload your Gov. ID for security validation. Max size: 5MB.
                    </p>
                    <label className="block">
                        <span className="sr-only">Choose document file</span>
                        <input 
                            key={fileInputKey} // Key change karne se input reset ho jaayega
                            type="file" 
                            accept="image/png, image/jpeg, application/pdf"
                            onChange={handleFileChange} 
                            required={currentDBStatus !== 'submitted'} // Agar pehli baar submit kar rahe hain
                            className="block w-full text-sm text-gray-500
                                file:mr-4 file:py-2 file:px-4
                                file:rounded-full file:border-0
                                file:text-sm file:font-semibold
